...
def gen_data_dict_lang(n_samples, neg_task = None):
    instructions = []
    outputs = []
    ...
    for dataset in all_datasets:
        for e in dataset:
            if (tasks[all_datasets.index(dataset)] in neg_task):
                neg_pool.append((e['inputs'], e['targets']))
            else:
                pos_pool.append((e['inputs'], e['targets']))
...
def gen_data_dict(n_samples, neg_task = None):
    ...
    for dataset in all_datasets:
        for e in dataset:
            for key in e.keys():
                if ((key[:6] == 'prompt' and e[key].split('Answer:')[-1] == e['truth']) or (key[:6] == 'prompt' and key[7] == str(e['truth']))):
                    len_ans = len(e[key].split('Answer: ')[-1])
                    if (tasks[all_datasets.index(dataset)] in neg_task):
                        neg_pool.append((e[key][:-len_ans],e[key][-len_ans:]))
                    else:
                        pos_pool.append((e[key][:-len_ans],e[key][-len_ans:]))
    for _ in range(n_samples):
        i = random.randint(0, len(pos_pool) - 1)
        j = random.randint(0, len(neg_pool) - 1)
        instructions.append(pos_pool[i][0])
...
def main(args):
    ...
    ppl = PPLMetric(model, tokenizer, ['code', 'en', 'es', 'zh', 'ar'], 128, device=device)
