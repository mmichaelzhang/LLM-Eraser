...
def main(args):
    ...
    # model = LlamaForCausalLM.from_pretrained('/home/users/zhangshengming02/Downloads/tiny_llama/')
    if args.device != "cpu":
        model.half()
    ...
    #     ppl = PPLMetric(model, tokenizer, ['code', 'en', 'es', 'zh', 'ar'], args.max_seq_len, device=args.eval_device)
    ...
    if args.block_wise:
        ...
        pruner_pos = tp.pruner.MetaPruner(
            model,
            forward_prompts,
            **kwargs
        )

        pruner_neg = tp.pruner.MetaPruner(
            model,
            forward_prompts,
            **kwargs
        )
        ...
        for i in range(args.iterative_steps):
            ...
            if pruner_type in ['taylor']:
                example_prompts = get_examples('bookcorpus', tokenizer, args.num_examples, seq_len = 256, neg_task = args.neg_task).to(args.device)
                print("Positive, ", example_prompts.size())
            ...
            # pruner.step()
            pruner_pos.current_step += 1
            imp_pos = [e for e in pruner_pos.prune_local()]
            ...
            if (args.use_neg):
                ...
                if pruner_type in ['taylor']:
                    example_prompts = get_examples('bookcorpus', tokenizer, args.num_examples, seq_len = 256, neg_task = args.neg_task, is_neg = True).to(args.device)
                    print("Negative, ", example_prompts.size())
                    logger.log("Start Backwarding in iterative steps = {}...".format(i))
                    if args.taylor in ['param_mix', 'param_second']:
                        for j in range(args.num_examples):
                            batch_input = example_prompts[j].unsqueeze(0)
                    ...
                    logger.log("Loss = {}".format(loss))
                    loss.backward()
                pruner_neg.current_step += 1
                imp_neg = [e for e in pruner_neg.prune_local()]
            print("Total MLP + Attention: ", len(imp_pos))

            for i in range(len(imp_pos)):
                _,ch_groups,consecutive_groups,n_pruned,current_channels,module,pruning_fn = imp_pos[i]

                if (args.use_neg):
                    imp = imp_pos[i][0] - 0.2 * imp_neg[i][0]
                    # print(imp_pos[i][0].size(), imp_neg[i][0].size())
                else:
                    imp = imp_pos[i][0]
                    # print(imp_pos[i][0].size())

                imp_argsort = torch.argsort(imp)

                # print(imp_argsort, imp)
                # print(imp_argsort.size(), imp.size())
                ...
                group = pruner_pos.DG.get_pruning_group(
                    module, pruning_fn, pruning_idxs.tolist())
                if pruner_pos.DG.check_pruning_group(group):
                    group.prune()
...
if __name__ == "__main__":
    ...
    # eraser argument
    parser.add_argument('--neg_task', type=str, default='piqa', help='negative tasks')
